...
async function main() {
  ...
  figures.forEach(([name, region, role, era, clue], index) => {
    ...
    questions.push(q(`${id}-region`, 'history', 1 + ((index + 2) % 3), `À quel espace historique rattache-t-on ${name} ?`, rotateChoices(regions, region), 0, `${name} est rattaché ici à ${region}.`, ['histoire', 'personnage-historique', 'geographie', 'banque-personnages'], `https://fr.wikipedia.org/w/index.php?search=${encodeURIComponent(name)}`));
